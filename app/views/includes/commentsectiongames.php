...
//Pagination
foreach ($data["countgamecomments"] as $count){
	$i += 1;
	$setPage += 1;
	$iT += 1;
		...
		if($countLimit < 16){
			...
			if($i == 11){
				...
				if($data["getpage"] == 1){echo "<a class='paginationTextBold'>" . 1 . "</a>";}else{
					echo "<a href='?page=" . $prev . "#comments'><img class='paginationBtn' src='" . URLROOT . "/public/graphic/global/pointerLeft.svg' /></a>";
					echo "<a class='paginationText' href='?page=" . 1 . "#comments'>" . 1 . "</a>";
				}
			}
			...
			if($setPage > 10){
				...
				if($data["getpage"] == ($i-1+10)/10){echo "<a class='paginationTextBold'>" . ($i-1+10)/10 . "</a>";}else{
					echo "<a class='paginationText' href='?page=" . ($i-1+10)/10 . "#comments'>" . ($i-1+10)/10 . "</a>";
				}
			}
		}
	}
...
//Display last page
if($data["getpage"] <= 15 && $totalPages > 16){echo "<a class='paginationText' href='?page=" . $totalPages . "#comments'>" . "..." . $totalPages . "</a>";}
if($data["getpage"] < $totalPages-8 && $data["getpage"] > 15){
	echo "<a class='paginationText' href='?page=" . $totalPages . "#comments'>" . "..." . $totalPages . "</a>";
}

//Set next button
if($setNextButton == true){
	if($data["pagectrl"] > $data["getpage"]){echo "<a href='?page=" . $next . "#comments'><img class='paginationBtn' src='" . URLROOT . "/public/graphic/global/pointerRight.svg' /></a>";}
}
